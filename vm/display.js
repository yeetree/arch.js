...
//Arch.JS 16 Bit Video Driver
buf=Array(canvas.width*canvas.height).fill(43)
function demo()
{

    RefreshVideo(ctx, buf, colors)
}

function RefreshVideo(ct, buffer, pallete)
{
    oldbuf=ct.getImageData(0,0,canvas.width,canvas.height);
    newbuf=[];
    for(i=0; i<oldbuf.data.length/4; i++)
    {
        pxcol=hex2rgb(pallete[buffer[i]])
        newbuf.push(pxcol.r)
        newbuf.push(pxcol.g)
        newbuf.push(pxcol.b)
        newbuf.push(255)
